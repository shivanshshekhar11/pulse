...
export function FlagDialog({
  ...
  const [type, setType] = useState<'boolean' | 'string' | 'number' | 'json'>(
  ...
  const keyError =
    key && !/^[a-z][a-z0-9_]*$/.test(key) ? 'invalid format' : undefined;
  ...
  return (
    <Dialog open={open} onClose={onClose} size="lg">
      <DialogHeader
        ...
        title={
          mode === 'create' ? 'create feature flag' : `edit ${initial?.key}`
        }
      ...
      <DialogBody className="space-y-5">
        ...
        <Field label="description" hint="optional">
          <Textarea
            placeholder="What does this flag control? Who should know?"
            value={description}
              ...
              return (
                ...
                >
                  <Icon
                    ...
                    style={{ color: active ? '#8be36b' : '#6b7a85' }}
                  />
        ...
        <Field label="default value" hint="returned when no rule matches">
          <Input mono defaultValue={DEFAULT_VALUES[type]} key={type} />
        </Field>
        ...
        <Field label="tags" hint="press enter to add">
          <TagInput
            value={tags}
            onChange={setTags}
            placeholder="e.g. frontend, experiment"
          />
        </Field>
        ...
        {mode === 'create' && (
          <div className="rounded-md border border-info/30 bg-info/5 p-3 font-mono text-[11.5px] text-muted-foreground">
            ...
            <span className="text-foreground">disabled</span> by default in all
            environments. Toggle to roll out.
          </div>
        )}
      </DialogBody>
      <DialogFooter
        hint={
          mode === 'create'
            ? 'creates with version=1, audit logged'
            : 'creates new audit entry · increments version'
        }
      >
