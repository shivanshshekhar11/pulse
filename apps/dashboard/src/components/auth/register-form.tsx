...
import {
  Terminal, Mail, Lock, User as UserIcon, Eye, EyeOff, ArrowRight, Check,
} from 'lucide-react';
...
export function RegisterForm() {
  ...
  const strength = scorePw(password);
      ...
      <div className="flex-1 flex flex-col justify-center px-8 sm:px-16 lg:px-24 py-10 min-w-0">
        <Link href="/" className="flex items-center gap-2 self-start mb-12">
          ...
          <div className="font-mono leading-none">
            <div className="text-[16px] tracking-tight">pulse<span className="text-primary cursor-blink">_</span></div>
        ...
        <div className="max-w-[420px] w-full">
          ...
          <p className="font-mono text-[12.5px] text-muted-foreground mb-8">
            <span className="text-primary">$</span> pulse auth register
            <span className="text-dim"> # free tier · upgrade anytime</span>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Field label="full name" required>
              <Input autoFocus placeholder="Alex Kowalski" value={name} onChange={(e) => setName(e.target.value)} prefix={<UserIcon className="size-3.5 text-muted-foreground" />} required />
            </Field>
            <Field label="email" required>
              <Input type="email" placeholder="you@company.com" value={email} onChange={(e) => setEmail(e.target.value)} prefix={<Mail className="size-3.5 text-muted-foreground" />} required />
            </Field>
            <Field label="password" required hint="min 8 characters">
              <div className="relative">
                <Input type={showPw ? 'text' : 'password'} placeholder="create a strong password" value={password} onChange={(e) => setPassword(e.target.value)} prefix={<Lock className="size-3.5 text-muted-foreground" />} required />
            ...
            <button type="submit" disabled={loading || !email || !password || !name || !terms} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-md font-mono text-[13px] bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div className="hidden lg:flex w-[480px] xl:w-[560px] border-l border-border bg-surface-0 flex-col p-10 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <h2 className="text-[22px] leading-tight mb-8 max-w-[380px]">
          <span className="gradient-text">feature flags</span> that ship at the speed of your terminal.
        </h2>
        <ul className="space-y-3">
          {['p99 < 12ms global eval — three-tier SDK fallback', 'self-hostable · no SaaS lock-in · MIT licensed', 'terminal-first DX with first-class CLI + SDKs', 'audit log every change · optimistic locking built-in'].map((f) => (
